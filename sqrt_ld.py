from helper_func import get_such_num, is_perfect_square
...
def sqrt_long_div(num: int, precision: int = 5):
    ...
    # Change num to int if type(num) == float but num is integer
    ...
    if len(str(integer_part)) % 2 == 0:
        dd = [
            integer_part_str[i] + integer_part_str[i + 1]
            for i in range(0, len(integer_part_str), 2)
        ]
    # Integer Part is of odd length
        ...
        dd += [
            integer_part_str[i] + integer_part_str[i + 1]
            for i in range(1, len(integer_part_str), 2)
            if i < len(integer_part_str)
        ]
    ...
    if isinstance(num, float):
        ...
        if len(decimal_part_str) % 2 == 1:
            decimal_part_str += "0"
        after_dd = [
            decimal_part_str[i] + decimal_part_str[i + 1]
            for i in range(0, len(decimal_part_str), 2)
        ] + ["00" for _ in range(abs(precision - num_precision))]
    else:
    ...
    for i in range(len(after_dd)):
        if i == 0:
            ...
            after_dividend.append(new_dividend)
            new_divisor = get_such_num(
                new_dividend, after_divisor[-1] + after_quotient[-1]
            )
            after_divisor.append(new_divisor)
            after_quotient.append(new_divisor % 10)
            new_rem = new_dividend - after_divisor[-1] * after_quotient[-1]
    ...
    top_bar = (
        "-" * (len(quotient_str) + 10)
        if len(assumed_num) < len(quotient_str)
        else "-" * (len(assumed_num) + 10)
    )
    ...
    if len(" " * len(after_quotient)) + len(str(after_dividend[-1])) - len(_bar) > 0:
        _bar += "_" * (
            len(" " * len(after_quotient))
            + len(str(after_dividend[-1]))
            - len(_bar)
            + 5
        )
    ...
    for i in range(len(after_dividend)):
        ...
        print(
            ...
            + " |"
            + divisor_space
            + str(after_divisor[i] * after_quotient[i])
        )
...
if __name__ == "__main__":
    ...
    if not is_perfect_square(num):
        precision = input("Enter the precision (default = 5): ").strip()
